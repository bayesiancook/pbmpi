...
#ifndef AAMUTSELSBDPPROFILE_H
#define AAMUTSELSBDPPROFILE_H
...
class AAMutSelSBDPProfileProcess : public virtual MatrixSBDPProfileProcess, public virtual AAMutSelProfileProcess, public virtual GeneralPathSuffStatMatrixMixtureProfileProcess	{
	...
	AAMutSelSBDPProfileProcess() {}
	virtual ~AAMutSelSBDPProfileProcess() {}

	protected:

	void Create(int innsite, int indim)	{
		cerr << "In two-argument Create of AAMutSelSBDPProfileProcess. Should not be here.\n";
		exit(1);
	}
	...
	void CreateMatrix(int k)	{
		if (matrixarray[k])	{
			cerr << "error in AAMutSelSBDPProfileProcess: matrixarray is not 0\n";
			exit(1);
		}
