...
void PartitionedExpoConjugateGTRGammaPhyloProcess::GlobalUpdateRRSuffStat()	{
	...
	for(int p=0; p<Np; ++p)
	{
		for(i=0; i<workload; ++i) {
			rrsuffstatcount[p][i] = 0;
			rrsuffstatbeta[p][i] = 0.0;
		}
	}
	...
	int k,l;
	double x;
	unsigned char* bvector = new unsigned char[workload*Np*(sizeof(int)+sizeof(double))];

        for(i=1; i<nprocs; ++i) {
                MPI_Recv(bvector,workload*Np*(sizeof(int)+sizeof(double)),MPI_UNSIGNED_CHAR,i,TAG1,MPI_COMM_WORLD,&stat);
                ...
                for(int p = 0; p < Np; p++)
                {
					for(j=0; j<workload; ++j) {
							...
							for(k=sizeof(int)-1; k>=0; --k) {
									l = (l << 8) + bvector[sizeof(int)*(p*workload + j)+k];
							}
							rrsuffstatcount[p][j] += l;
					}
					for(j=0; j<workload; ++j) {
							memcpy(&x,&bvector[sizeof(int)*workload*Np+sizeof(double)*(p*workload + j)],sizeof(double));
							rrsuffstatbeta[p][j] += x;
					}
                }
        }
	delete[] bvector;
...
void PartitionedExpoConjugateGTRGammaPhyloProcess::SlaveUpdateRRSuffStat()	{
	...
	{
		for(i=0; i<workload; ++i) {
			convert(el_int,rrsuffstatcount[p][i]);
	...
	{
		for(i=0; i<workload; ++i) {
			convert(el_dbl,rrsuffstatbeta[p][i]);
