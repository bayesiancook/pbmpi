...
void PhyloProcess::SlaveExecute(MESSAGE signal)	{
    ...
    */
	...
	case WRITE_SUFFSTAT:
		SlaveWriteSuffStat();
		break;
	case WRITE_SUFFDISTAT:
		SlaveWriteSuffDiStat();
		break;
	case COUNTMAPPING:
		SlaveCountMapping();
		break;
...
void PhyloProcess::GlobalWriteSuffStat(string name){
	MESSAGE signal = WRITE_SUFFSTAT;
	MPI_Bcast(&signal,1,MPI_INT,0,MPI_COMM_WORLD);
...
void PhyloProcess::GlobalWriteSuffDiStat(string name){
	MESSAGE signal = WRITE_SUFFDISTAT;
	MPI_Bcast(&signal,1,MPI_INT,0,MPI_COMM_WORLD);
...
void PhyloProcess::SlaveWriteSuffStat(){
...
void PhyloProcess::WriteSuffStat(ostream& os, const Link* from, int i){
	if(from->isLeaf()){
	}
	else{
		for (const Link* link=from->Next(); link!=from; link=link->Next()){
			WriteSuffStat(os, link->Out(), i);
		}
	}
	if(from->isRoot()){
		os << "\n";   
