...
int PhyloProcess::GlobalMappingStats(CodonStateSpace* codonstatespace)	{

	assert(myid==0);
	MESSAGE signal = MAPPINGSTATS;
	MPI_Status stat;
	MPI_Bcast(&signal,1,MPI_INT,0,MPI_COMM_WORLD);

	int code = codonstatespace->GetGeneticCodeType();
	MPI_Bcast(&code,1,MPI_INT,0,MPI_COMM_WORLD);

	int i, count, totalcount=0;
	for (i=1; i<nprocs; ++i)	{
		MPI_Recv(&count,1,MPI_INT,i,TAG1,MPI_COMM_WORLD, &stat);
		totalcount += count;
	}
	return totalcount;
...
void PhyloProcess::SlaveMappingStats(int code)	{
	...
	if (code==0)	{
		...
		cerr << "Sorry, this isn't well done yet.\n";
		exit(1);
	}
	CodonStateSpace* codonstatespace = new CodonStateSpace(codetype);
	
	int nonsyn = NonSynMapping(codonstatespace);
	MPI_Send(&nonsyn,1,MPI_INT,0,TAG1,MPI_COMM_WORLD);

}

void PhyloProcess::ReadMapStats(string name, int burnin, int every, int until, CodonStateSpace* codonstatespace){
  	ifstream is((name + ".chain").c_str());
	if (!is)	{
		cerr << "error: no .chain file found\n";
		exit(1);
	...
	while (i < until)	{
		...
		GlobalCollapse();
		// write posterior
		obs = GlobalMappingStats(codonstatespace);
		ospost << (double) (obs) / GetNsite() << "\n";

		/*
		GlobalUpdateSiteProfileSuffStat();
		int pairobs = 0;
		for (int i=0; i<GetNsite(); i++)	{
			for (int a=0; a<GetNstate(i); a++)	{
				for (int b=0; b<GetNstate(i); b++)	{
					pairobs += sitepaircount[i][pair<int,int>(a,b)];
				}
			}
		}
		*/
		...
		pred = GlobalMappingStats(codonstatespace);
		ospred << (double) (pred) / GetNsite() << "\n";
...
int PhyloProcess::NonSynMapping(CodonStateSpace* codonstatespace)	{

	int total = 0;	
	for(int i = sitemin; i < sitemax; i++){
		total += CountNonSynMapping(GetRoot(), i, codonstatespace);
	}
	return total;
}

int PhyloProcess::CountNonSynMapping(const Link* from, int i, CodonStateSpace* codonstatespace){
	int count = 0;
	if(!from->isLeaf()){
		for (const Link* link=from->Next(); link!=from; link=link->Next()){
			count += CountNonSynMapping(link->Out(), i, codonstatespace);
		}
	}
	if(!from->isRoot()){
		...
		while (plink != mybsp->Last())	{
			count++;
			/*
			if (! codonstatespace->Synonymous(plink->GetState(), plink->Next()->GetState()) )	{
				count++;
			}
			*/
			plink = plink->Next();
